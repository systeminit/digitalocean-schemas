async function main({
    ...
    // Fetch all VPCs using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching VPC page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/vpcs?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.vpcs) && listResponse.vpcs.length > 0) {
                console.log(`found ${listResponse.vpcs.length} VPCs on page ${page}`);
                resourceList = _.union(resourceList, listResponse.vpcs);
            }
        ...
        return {
            status: "error",
            message: `VPC list error: ${error.message}`
        };
    }

    console.log(`total VPCs collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per VPC into SI components.
    let importCount = 0;
    for (const vpc of resourceList) {
        const resourceId = vpc.id;
        console.log(`importing VPC with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for VPC ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean VPC",
            properties,
