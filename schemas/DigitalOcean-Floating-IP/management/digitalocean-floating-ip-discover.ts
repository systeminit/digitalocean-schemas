async function main({
    ...
    // Fetch all floating IPs using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching floating IP page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/floating_ips?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.floating_ips) && listResponse.floating_ips.length > 0) {
                console.log(`found ${listResponse.floating_ips.length} floating IPs on page ${page}`);
                resourceList = _.union(resourceList, listResponse.floating_ips);
            }
        ...
        return {
            status: "error",
            message: `Floating IP list error: ${error.message}`
        };
    }

    console.log(`total floating IPs collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per floating IP into SI components.
    let importCount = 0;
    for (const floatingIp of resourceList) {
        const resourceId = floatingIp.ip;
        console.log(`importing floating IP with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for floating IP ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Floating IP",
            properties,
