async function main({
  ...
  if (!resourceId) {
    return {
      status: "error",
      message: "resource ID not provided; please specify a volume snapshot ID to import",
    };
  }

  const response = await fetch(`https://api.digitalocean.com/v2/snapshots/${resourceId}`, {
    method: "GET",
    headers: {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to fetch volume snapshot; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }
  ...
  console.log("volume snapshot imported and attributes prepared for component creation");
