async function main(component: Input): Promise<Output> {
  ...
  const response = await fetch("https://api.digitalocean.com/v2/domains", {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to create domain; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const resourceId = responseJson.domain?.id;
  ...
  if (resourceId) {
    ...
    return {
      message: "Failed to extract domain id from response",
      status: "error",
    };
  }
}
