async function main(component: Input): Promise<Output> {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to create ssh_key; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const resourceId = responseJson.ssh_key?.id;
  ...
  if (resourceId) {
    ...
    return {
      message: "Failed to extract ssh_key id from response",
      status: "error",
    };
  }
}
