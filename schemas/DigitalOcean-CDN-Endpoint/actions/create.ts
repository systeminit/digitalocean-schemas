async function main(component: Input): Promise<Output> {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to create cdn_endpoint; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const resourceId = responseJson.endpoint?.id;

  if (resourceId) {
    return {
      resourceId: resourceId.toString(),
      status: "ok",
      payload: responseJson.endpoint,
    };
  } else {
    return {
      message: "Failed to extract cdn_endpoint id from response",
      status: "error",
    };
  }
}
