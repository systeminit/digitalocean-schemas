async function main({
  ...
  if (!resourceId) {
    return {
      status: "error",
      message: "resource ID not provided; please specify an SSH key ID to import",
    };
  }
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to fetch SSH key; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const sshKey = responseJson.ssh_key;
  if (!sshKey) {
    return {
      status: "error",
      message: "Failed to extract SSH key data from response",
    };
  }
  ...
  console.log("SSH key imported and attributes prepared for component creation");
