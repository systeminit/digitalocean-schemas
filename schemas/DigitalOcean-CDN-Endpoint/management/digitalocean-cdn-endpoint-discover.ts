async function main({
    ...
    // Fetch all CDN endpoints using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching CDN endpoint page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/cdn/endpoints?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.endpoints) && listResponse.endpoints.length > 0) {
                console.log(`found ${listResponse.endpoints.length} endpoints on page ${page}`);
                resourceList = _.union(resourceList, listResponse.endpoints);
            }
        ...
        return {
            status: "error",
            message: `CDN endpoint list error: ${error.message}`
        };
    }

    console.log(`total CDN endpoints collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per endpoint into SI components.
    let importCount = 0;
    for (const endpoint of resourceList) {
        const resourceId = endpoint.id.toString();
        console.log(`importing CDN endpoint with resource ID ${resourceId}`);
        ...
        const properties = {
            si: {
              resourceId,
              name: endpoint.origin || endpoint.id,
            },
            domain: domainFields,
            resource: endpoint,
        };
        ...
        console.log(`final attributes for CDN endpoint ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean CDN Endpoint",
            properties,
