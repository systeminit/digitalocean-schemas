async function main(component: Input): Promise < Output > {
    ...
    if (!response.ok) {
        ...
        return {
            status: "error",
            message: `Unable to create domain; API returned ${response.status}
  ${response.statusText}: ${errorText}`,
        };
    }

    const responseJson = await response.json();
    const resourceId = responseJson.domain?.name;
    ...
    if (resourceId) {
        ...
        return {
            message: "Failed to extract domain name from response",
            status: "error",
        };
    }
}