async function main({
    ...
    // Fetch all tags using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching tag page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/tags?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.tags) && listResponse.tags.length > 0) {
                console.log(`found ${listResponse.tags.length} tags on page ${page}`);
                resourceList = _.union(resourceList, listResponse.tags);
            }
        ...
        return {
            status: "error",
            message: `Tag list error: ${error.message}`
        };
    }

    console.log(`total tags collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per tag into SI components.
    let importCount = 0;
    for (const tag of resourceList) {
        const resourceId = tag.name;
        console.log(`importing tag with resource ID ${resourceId}`);

        // Map API fields to domain properties
        const domainFields = mapApiFieldToDomain(tag, fieldMappings);
        ...
        console.log(`final attributes for tag ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Tag",
            properties,
