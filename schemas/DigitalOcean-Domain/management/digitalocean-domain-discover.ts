async function main({
    ...
    // Fetch all domains using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching domain page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/domains?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.domains) && listResponse.domains.length > 0) {
                console.log(`found ${listResponse.domains.length} domains on page ${page}`);
                resourceList = _.union(resourceList, listResponse.domains);
            }
        ...
        return {
            status: "error",
            message: `Domain list error: ${error.message}`
        };
    }

    console.log(`total domains collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per domain into SI components.
    let importCount = 0;
    for (const domain of resourceList) {
        const resourceId = domain.name;
        console.log(`importing domain with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for domain ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Domain",
            properties,
