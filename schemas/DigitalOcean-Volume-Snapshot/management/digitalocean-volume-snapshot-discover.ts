async function main({
    ...
    // Fetch all volume snapshots using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching volume snapshot page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/snapshots?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.snapshots) && listResponse.snapshots.length > 0) {
                console.log(`found ${listResponse.snapshots.length} snapshots on page ${page}`);
                resourceList = _.union(resourceList, listResponse.snapshots);
            }
        ...
        return {
            status: "error",
            message: `Volume snapshot list error: ${error.message}`
        };
    }

    console.log(`total volume snapshots collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per snapshot into SI components.
    let importCount = 0;
    for (const snapshot of resourceList) {
        const resourceId = snapshot.id;
        console.log(`importing volume snapshot with resource ID ${resourceId}`);

        // Map API fields to domain properties
        const domainFields = mapApiFieldToDomain(snapshot, fieldMappings);
        ...
        console.log(`final attributes for volume snapshot ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Volume Snapshot",
            properties,
