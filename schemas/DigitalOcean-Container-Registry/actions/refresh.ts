async function main(component: Input): Promise<Output> {
  ...
  if (response.status === 404) {
    return {
      status: "error",
      message: `Container Registry with ID ${resourceId} not found`,
    };
  }
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to read registry; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();

  if (responseJson.registry) {
    return {
      status: "ok",
