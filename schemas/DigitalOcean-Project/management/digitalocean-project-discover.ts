async function main({
    ...
    // Fetch all projects using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching project page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/projects?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.projects) && listResponse.projects.length > 0) {
                console.log(`found ${listResponse.projects.length} projects on page ${page}`);
                resourceList = _.union(resourceList, listResponse.projects);
            }
        ...
        return {
            status: "error",
            message: `Project list error: ${error.message}`
        };
    }

    console.log(`total projects collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per project into SI components.
    let importCount = 0;
    for (const project of resourceList) {
        const resourceId = project.id;
        console.log(`importing project with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for project ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Project",
            properties,
