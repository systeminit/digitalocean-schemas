async function main(component: Input): Promise<Output> {
  ...
  const response = await fetch("https://api.digitalocean.com/v2/projects", {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to create project; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const resourceId = responseJson.project?.id;
  ...
  if (resourceId) {
    ...
    return {
      message: "Failed to extract project id from response",
      status: "error",
    };
  }
}
