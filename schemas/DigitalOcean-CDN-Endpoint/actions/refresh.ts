async function main(component: Input): Promise<Output> {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to read cdn_endpoint; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }
  ...
  if (responseJson.endpoint) {
    ...
    return {
      status: "error",
      message: "Failed to extract cdn_endpoint data from response",
    };
  }
}
