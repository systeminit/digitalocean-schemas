async function main({
    ...
    // Fetch all volumes using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching volume page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/volumes?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.volumes) && listResponse.volumes.length > 0) {
                console.log(`found ${listResponse.volumes.length} volumes on page ${page}`);
                resourceList = _.union(resourceList, listResponse.volumes);
            }
        ...
        return {
            status: "error",
            message: `Volume list error: ${error.message}`
        };
    }

    console.log(`total volumes collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per volume into SI components.
    let importCount = 0;
    for (const volume of resourceList) {
        const resourceId = volume.id;
        console.log(`importing volume with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for volume ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Volume",
            properties,
