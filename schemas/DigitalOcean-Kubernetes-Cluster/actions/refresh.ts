async function main(component: Input): Promise<Output> {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to read kubernetes_cluster; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }
  ...
  if (responseJson.kubernetes_cluster) {
    ...
    return {
      status: "error",
      message: "Failed to extract kubernetes_cluster data from response",
    };
  }
}
