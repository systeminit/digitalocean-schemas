async function main({
    ...
    // Fetch all custom images using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching custom image page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/images?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.images) && listResponse.images.length > 0) {
                console.log(`found ${listResponse.images.length} images on page ${page}`);
                resourceList = _.union(resourceList, listResponse.images);
            }
        ...
        return {
            status: "error",
            message: `Custom image list error: ${error.message}`
        };
    }

    console.log(`total custom images collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per image into SI components.
    let importCount = 0;
    for (const image of resourceList) {
        const resourceId = image.id.toString();
        console.log(`importing custom image with resource ID ${resourceId}`);
        ...
        console.log(`final attributes for custom image ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Custom Image",
            properties,
