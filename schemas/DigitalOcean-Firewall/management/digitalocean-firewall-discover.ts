async function main({
    ...
    // Fetch all firewalls using pages.
    try {
        let page = 1;
        let perPage = 200;
        let hasNextPage = true;
        while (hasNextPage) {
            console.log(`fetching firewall page ${page} (max ${perPage} per page)`);
            const listResponse = await doApiFetch(`/firewalls?per_page=${perPage}&page=${page}`);

            if (Array.isArray(listResponse?.firewalls) && listResponse.firewalls.length > 0) {
                console.log(`found ${listResponse.firewalls.length} firewalls on page ${page}`);
                resourceList = _.union(resourceList, listResponse.firewalls);
            }
        ...
        return {
            status: "error",
            message: `Firewall list error: ${error.message}`
        };
    }

    console.log(`total firewalls collected: ${resourceList.length}`);
    ...
    // Convert the raw API response per firewall into SI components.
    let importCount = 0;
    for (const firewall of resourceList) {
        const resourceId = firewall.id;
        console.log(`importing firewall with resource ID ${resourceId}`);

        // Map API fields to domain properties
        const domainFields = mapApiFieldToDomain(firewall, fieldMappings);

        const properties = {
            si: {
              resourceId,
              name: firewall.name || firewall.id,
            },
            domain: domainFields,
            resource: firewall,
        };
        ...
        console.log(`final attributes for firewall ${resourceId}`, newAttributes);

        create[resourceId] = {
            kind: "DigitalOcean Firewall",
            properties,
