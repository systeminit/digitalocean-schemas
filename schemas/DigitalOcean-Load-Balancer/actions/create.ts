async function main(component: Input): Promise<Output> {
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to create load_balancer; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }

  const responseJson = await response.json();
  const resourceId = responseJson.load_balancer?.id;
  ...
  if (resourceId) {
    ...
    return {
      message: "Failed to extract load_balancer id from response",
      status: "error",
    };
  }
}
