async function main({
  ...
  if (!resourceId) {
    return {
      status: "error",
      message: "resource ID not provided; please specify a monitoring alert policy UUID to import",
    };
  }
  ...
  if (response.status === 404) {
    return {
      status: "error",
      message: `Monitoring Alert Policy with UUID ${resourceId} not found`,
    };
  }
  ...
  if (!response.ok) {
    ...
    return {
      status: "error",
      message: `Unable to fetch monitoring alert policy; API returned ${response.status} ${response.statusText}: ${errorText}`,
    };
  }
  ...
  console.log("monitoring alert policy imported and attributes prepared for component creation");
